import LetterGlitch from "../React/LetterGlitch.jsx";
import LogoWall from "./logoWall.jsx";
import SkillsList from "../React/SkillsList.jsx";
...
const Home = () => (
  <section className="text-[var(--white)] mt-12 md:mt-0" id="home">
      ...
      <div className="text-left space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center space-y-4 lg:space-y-0 lg:space-x-8 md:gap-4">
          ...
          <p className="text-md md:text-2xl text-[var(--white-icon)]">
            Enthusiastic University of Colombo School of Computing undergraduate
            passionate about web development, Java, and modern tech stacks.
          </p>
        </div>
        <div className="flex justify-start gap-2 pt-3 md:pt-6">
          <a
            target="_blank"
            href="https://github.com/isharaudayamali"
            aria-label="GitHub"
          ...
          <a
            target="_blank"
            href="https://www.linkedin.com/in/ishara-udayamali"
            aria-label="LinkedIn"
            className="text-[var(--white-icon)] hover:text-white transition duration-300 ease-in-out border border-1 border-[var(--white-icon-tr)] p-3 rounded-xl bg-[#1414149c] hover:bg-[var(--white-icon-tr)]"
          >
            <svg
    ...
    <style>{`.shiny-sec { background: linear-gradient(135deg, #a476ff 25%, #eee5ff 50%, #a476ff 75%); background-size: 400% 100%; -webkit-background-clip: text; background-clip: text; color: transparent; animation: shine 3s linear infinite; } @keyframes shine { 0% { background-position: 100% 50%; } 30%, 70% { background-position: 0% 50%; } }`}</style>
  </section>
);
export default Home;
